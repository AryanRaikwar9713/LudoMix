          ...
          child: Consumer<Ludo2v2>(
            builder: (context, value, child) {
              ...
              for (int i = 0; i < players.length; i++) {
                ...
                if (value.offlinePlayers.contains(player.type)) {
                  offlinePlayersList.add(player);
                  continue;
                }
                  ...
                  playersPawn.addAll(
                    pawnsValue.map((e) {
                      ...
                      return AnimatedPositioned(
                        ...
                        duration: const Duration(milliseconds: 100), // Reduced from 200ms for faster animation
                        child: e,
                      );
                    }),
                  );
              ...
              // Add "Offline" text for offline players at their home positions
              for (var offlinePlayer in offlinePlayersList) {
                // Show "Offline" text at the first home position
                if (offlinePlayer.homePath.isNotEmpty) {
                  playersPawn.add(
                    Positioned(
                      left: LudoPath.stepBox(
                          ludoBoard(context), offlinePlayer.homePath[0][0]),
                      top: LudoPath.stepBox(
                          ludoBoard(context), offlinePlayer.homePath[0][1]),
                      width: boxStepSize(context) * 2.5,
                      height: boxStepSize(context),
                      child: Container(
                        decoration: BoxDecoration(
...
class _GameScreen2v2State extends State<GameScreen2v2> {
  ...
  bool isLoading = true;
  bool _hasSettledMatch = false; // Track if match has been settled
  ...
  // Call settle match API when game ends
  Future<void> _settleMatch(String winnerId) async {
    if (_hasSettledMatch) return; // Prevent multiple calls
    
    try {
      final url = 'https://ludo.eventsystem.online/api/board/settle_match.php';
      final response = await http.post(
        Uri.parse(url),
        headers: {'Content-Type': 'application/json'},
        body: jsonEncode({
          'match_id': int.tryParse(widget.matchId) ?? widget.matchId,
          'winner_id': int.tryParse(winnerId) ?? winnerId,
        }),
      ).timeout(
      ...
      debugPrint('Settle match response: ${response.statusCode} - ${response.body}');
      
      if (response.statusCode == 200) {
        final data = jsonDecode(response.body);
        if (data['status'] == 'success') {
          _hasSettledMatch = true;
          debugPrint('Match settled successfully: ${data['message']}');
          if (mounted) {
            ScaffoldMessenger.of(context).showSnackBar(
              SnackBar(
                content: Text(data['message'] ?? 'Match settled successfully'),
          ...
          debugPrint('Settle match error: ${data['message']}');
        }
      } else {
        debugPrint('Settle match failed with status: ${response.statusCode}');
      }
    } catch (e) {
      debugPrint('Error settling match: $e');
      // Don't show error to user as game is already over
    }
  }
            ...
            children: [
              ...
              Consumer<Ludo2v2>(
                builder: (context, value, child) {
                  ...
                  if (isGameOver) {
                    // Get winner info
                    final winnerColor = value.winners.first;
                    Map<String, dynamic>? winnerPlayer;
                    String winnerName = winnerColor.name.toUpperCase();
                    String winnerImage = '';
                    String? winnerId;
                    
                    // Find winner player based on color
                    if (winnerColor == LudoPlayerType.green) {
                      winnerPlayer = p1;
                    ...
                    if (winnerPlayer != null && winnerPlayer['name'] != null) {
                      winnerName = winnerPlayer['name'] ?? winnerColor.name.toUpperCase();
                      final profilePic = winnerPlayer['profilePic'] ?? '';
                      winnerImage = _fixProfilePictureUrl(profilePic.toString());
                      if (winnerImage.isEmpty) {
                        winnerImage = '${ConstRes.base}/storage/profile/profile.png';
                      }
                      // Get winner's user ID
                      winnerId = winnerPlayer['id']?.toString();
                    }
                    
                    // Call settle match API when game ends (only once)
                    if (winnerId != null && !_hasSettledMatch) {
                      WidgetsBinding.instance.addPostFrameCallback((_) {
                        _settleMatch(winnerId!);
                      });
                    }
                    
                    // Calculate winning amount (assuming default entry fee, can be enhanced with match data)
                    final entryFee = 12; // Default, can be fetched from match data
                    final winningAmount = widget.playersRequired == 2
                        ? (entryFee == 12 ? 20 : entryFee == 17 ? 30 : entryFee == 24 ? 40 : entryFee == 30 ? 50 : entryFee == 36 ? 62 : entryFee * 2)
                        : entryFee * (widget.playersRequired - 1);
                                  ...
                                  children: [
                                    CircleAvatar(
                                      ...
                                      onBackgroundImageError: (e, stack) {
                                        debugPrint('Failed to load winner image: $e');
                                      },
                                    ),
                  ...
                  children: [
                    ...
                    Positioned(
                      ...
                      child: _buildUserProfile(
                        ...
                        isOffline: offlinePlayers.contains(LudoPlayerType.green),
                      ),
                    ),
                    ...
                    Positioned(
                      ...
                      child: _buildUserProfile(
                        ...
                        isOffline: offlinePlayers.contains(LudoPlayerType.yellow),
                      ),
                    ),
  ...
  Widget _buildUserProfile(String username, String imageUrl, {
    ...
    if (isEmpty || username.isEmpty) {
      return Container(
        ...
        alignment: Alignment.center,
        child: Text('—', style: TextStyle(color: Colors.grey[600], fontSize: 20)),
      );
    }
    
    // Get color based on player type and turn
    Color cardColor = isOffline 
        ? Colors.grey[700]!.withOpacity(0.5)
        : Colors.grey[800]!.withOpacity(0.7);
    Color borderColor = isOffline 
        ? Colors.grey.shade500
        : Colors.grey.shade600;
    
    if (isCurrentTurn && playerColor != null && !isOffline) {
      switch (playerColor) {
        case LudoPlayerType.green:
          cardColor = LudoColor.green.withOpacity(0.9);
          borderColor = LudoColor.green;
          break;
        case LudoPlayerType.yellow:
          cardColor = LudoColor.yellow.withOpacity(0.9);
          borderColor = LudoColor.yellow;
          break;
        case LudoPlayerType.blue:
          cardColor = LudoColor.blue.withOpacity(0.9);
          borderColor = LudoColor.blue;
          break;
        case LudoPlayerType.red:
          cardColor = LudoColor.red.withOpacity(0.9);
